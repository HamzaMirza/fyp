...
function callsRedirectionOnAdminPages(filename) {
	...
	if (filename == 'admin.php?questions_view')
		...
		initializeDesignationSelectBox();
		initializeRolesSelectBox();
	}
}
...
function generate_table_applicants() {
	...
	for (var i = 0; i < displayRecords.length; i++) {
		...
		tr.append("<td>" +
			" <a class=\"btn btn-yellow cvButton\" download href=\"" + base_url_laravel + " " + displayRecords[i].cv + "\" > Download CV </a>" +
			"</td>");
...
function generate_table_admins() {
	...
	for (var i = 0; i < displayRecords.length; i++) {
		...
		tr.append("<td>" +
			" <a class=\"btn btn-yellow \" onclick=\"openModel(" + displayRecords[i].id + ",'" + displayRecords[i].email + "')\" > Delete </a>" +
			"</td>");
...
// add vacancies
function reqAddVacancy() {
	console.log("EE");
	if (checkForm()) {
			console.log("c",user.cid,
			"t",$("input[name=title]").val(),
			'a',$("input[name=avgsalary]").val(),
			'ld',$("input[name=lastDate]").val(),
			 'd',$("#designation").children("option:selected").val(),
			 'r',$("#role").children("option:selected").val(),
			's',$("input[name=seats]").val(),
			'e',$("input[name=minexperience]").val(),
			 's',$("#status").children("option:selected").val(),
			 'c',$("#city").children("option:selected").val());
		ajaxCall_vacancies_add();
	}
}

function ajaxCall_vacancies_add() {
	$.ajax({
		type: 'POST',
		url: base_url + 'uploadvacancies.php',
		dataType: 'json',
		data: {
			'cid': user.cid,
			'title': $("input[name=title]").val(),
			'avgsalary': $("input[name=avgsalary]").val(),
			'lastDate': $("input[name=lastDate]").val(),
			'did': $("#designation").children("option:selected").val(),
			'rid': $("#role").children("option:selected").val(),
			'seats': $("input[name=seats]").val(),
			'minexperience': $("input[name=minexperience]").val(),
			'state': $("#status").children("option:selected").val(),
		...
		success: function (data) {
			...
			if (data.success == 0)
				...
				$("input[name=avgsalary]").val('');
				$("input[name=lastDate]").val('');
				$("input[name=seats]").val('');
				$("input[name=minexperience]").val('');
...
//Designation
function initializeDesignationSelectBox() {
	var select = document.getElementById("designation");
	ajaxCall_getDesignations().then((data) => {
		if (data != 0) {
...
async function ajaxCall_getDesignations() {
	let designations = 0;
	await $.ajax({
		type: 'POST',
		url: base_url + 'designations.php',
		dataType: 'json',
		success: function (data) {
			console.log("", data);
			if (data.success == 0)
...
//Roles
function initializeRolesSelectBox() {
	var select = document.getElementById("role");
	ajaxCall_getRoles().then((data) => {
		if (data != 0) {
			for (var j = 0; j < data.length; j++)
				select.options[select.options.length] = new Option(data[j]["name"], data[j]["id"]);
		}
...
async function ajaxCall_getRoles() {
	let roles = 0;
	await $.ajax({
		type: 'POST',
		url: base_url + 'roles.php',
		dataType: 'json',
		success: function (data) {
			console.log("", data);
			if (data.success == 0)
